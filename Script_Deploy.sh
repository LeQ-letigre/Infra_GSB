...
# === [IDEMPOTENCE] Vérifie si le conteneur existe déjà ===
if pct status $CTID &>/dev/null; then
  echo "[!] Le conteneur $CTID existe déjà. Tentative de nettoyage Terraform distant..."

  IP_ADDR="${IP%%/*}"  # Nettoie l'adresse CIDR
  echo "[+] Attente que le conteneur réponde en SSH..."
  until ping -c1 -W1 "$IP_ADDR" > /dev/null 2>&1; do
    echo "⏳ En attente du conteneur $CTNAME ($IP_ADDR)..."
    sleep 1
  done

  echo "[+] Connexion SSH et exécution de terraform destroy..."
  ssh -T -o StrictHostKeyChecking=no -i "$SSH_KEY_PATH" root@"$IP_ADDR" <<EOF
cd /Infra_GSB/Terraform
if [ -f .terraform.lock.hcl ]; then
  terraform init -input=false
  terraform destroy -auto-approve || echo "⚠️ terraform destroy a échoué ou partiellement échoué"
else
  echo "⚠️ Aucun projet Terraform trouvé. Rien à détruire."
fi
EOF
...
# === Attribution du rôle PVEAdmin sur / ===
...
proxmox_api_token_id    = "\$TOKEN_NAME"
